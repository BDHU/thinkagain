...
from thinkagain.dist import runtime
from thinkagain.dist import clear_replica_registry, get_replica_spec, reset_backend
...
def test_replica_round_robin_distribution():
    """Test that round-robin actually distributes calls."""
    call_log = []
    ...
    class Logger:
        ...
        def log(self):
            call_log.append(self.id)

    # We need to manually create instances with different IDs
    # Since deploy uses same args for all, we test distribution differently
    Logger.deploy(id=0)

    # Get multiple instances - should cycle through
    for _ in range(4):
        Logger.get().log()

    # All calls should be to the same instance (id=0) since all created with same args
    assert call_log == [0, 0, 0, 0]
...
def test_grpc_backend_end_to_end():
    ...
    import time
    from thinkagain.dist.backend.grpc import ReplicaRegistry, serve
    ...
    try:
        ...
        from thinkagain.dist import init
        from thinkagain.dist.replica import ReplicaSpec
